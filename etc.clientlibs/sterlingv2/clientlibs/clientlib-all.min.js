...
return!0}}
function setDatePickers(){function a(){var a=$.datepicker.formatDate("mm/dd/yy",new Date($("#hsdCheckInDate").val())),c=$.datepicker.formatDate("mm/dd/yy",new Date($("#hsdCheckOutDate").val())),d=filterResort($("#singleDestination").val());destinationValidation(d.resortID)&&((new Date(c)).getTime(),(new Date(a)).getTime(),getData("/bin/nodeutil.content__sterlingholidays__en__index__resorts-hotels__"+d.filename+"__jcr-content__par__resort_overall__data.json",getResortOverallData))}$("#corporateCode,#singleDestination,#hsdCheckInDate,#hsdCheckOutDate").focusout(function(){validateBooking(!1)});$(document).on("change",
...
function(){$(".check-availability").disableButton();a()});$(".check-availability").unbind("click");$(".check-availability").click(function(){a:{try{var b=filterResort($("#singleDestination").val());if(b&&"Sterling Nainital"==b.masterName&&(moment(moment($("#hsdCheckInDate").val())).isBetween("2023-07-16","2023-10-16")||moment(moment($("#hsdCheckOutDate").val())).isBetween("2023-07-16","2023-10-16"))){var c=!0;break a}}catch(d){}c=!1}if(c)return $("#error-notification1").text("We are sorry! Sterling Nainital is being upgraded. The resort will remain closed from 16th July to 15th October 2023 for renovation. We look forward to welcoming you to a beautifully upgraded resort come 16th October!"),
$("#error-notification1").show(),!1;$("#error-notification1").hide();$("#error-notification1").text("");if(isOfferAvaild&&getSessionData("offerMinDate")&&(c=Number(getSessionData("offerMinDate")),b=moment($("#hsdCheckInDate").val()),moment($("#hsdCheckOutDate").val()).diff(b,"days")<c))return $(".days-exceed-alert").show(),$("#bookingDateError").show(),$("#bookingDateError").text("Required minimum "+c+" nights to be selected."),!1;validateBooking(!0)&&validateBookingDates()&&($("#travellingWithPetChk").is(":checked")?
$("#isCorporate").prop("checked")?a():$("#petFAQModal").modal("show"):a())})}function clearBkRelatedSessionData(){deleteSessionData("packageFlag")}function offersAtTheResortInBookNow(a,b){$("#singleDestination").val();var c=filterResort($("#singleDestination").val());$("#singleDestination").val()&&(void 0==c?$("#chkAvb").attr("disabled",!0):filterOffers(c.value,a,b));return[]}
function filterOffers(a,b,c){selectedResortName=a;a=GlobalConstants.URL.HSD.GET_VALID_PROMOS+"."+a.toLowerCase();getData(a,filterOffersResult)}
...
function getOccupancyDetailsArr(a){var b=[];if(a)if(Array.isArray(a))for(var c=0;c<a.length;c++){var d={};var e="string"==typeof a[c]?JSON.parse(a[c]):a[c];d.roomId=e.roomID;d.maxOccupancyCount=e.maxOccupancyCount;d.baseOccupancyCount=e.baseOccupancyCount;d.extraOccupancyAdultCount=e.extraOccupancyAdultCount;d.extraOccupancyChildCount=e.extraOccupancyChildCount;b.push(d)}else d={},"string"===$.type(a)&&(a=JSON.parse(a)),d.roomId=a.roomID,d.maxOccupancyCount=a.maxOccupancyCount,d.baseOccupancyCount=
...
function getData(a,b){$.ajax({url:a,cache:!0,beforeSend:function(){return DATA_CACHE.hasOwnProperty(a)&&DATA_CACHE[a]?(b(DATA_CACHE[a]),!1):!0},complete:function(c,d){"success"==d&&(DATA_CACHE[a]=c);b(c)}})}function postData(a,b,c){c?$.ajax({type:"POST",url:a,dataType:"json",contentType:"application/json",data:JSON.stringify(c),cache:!1,success:function(a){b(a)}}):$.ajax({type:"POST",url:a,cache:!1,success:function(a){b(a)}})}
...
var makeActive=function(a){var b="EXPERIENCES;RESORTS;MEMBERSHIP;OFFERS;PACKAGES;EVENTS;BLOG;GIFT CARD;PICNICS;PARTNERSHIPS".split(";"),c=$(a).data("menutitle");if(c){c=c.toUpperCase();for(var d=0;d<b.length;d++)a=b[d],c===a&&setSessionData("selectedlink",a),""===c&&setSessionData("selectedlink","")}},goTovoDashboard=function(a){window.location=a&&"undefined"!=typeof a?PAGE.voDashboard+a:PAGE.voDashboard},goTovoFeedBackUrl=function(a){window.location=a&&"undefined"!=typeof a?PAGE.voDashboard+a:PAGE.voDashboard},
headLoggedInUserType="";function onBookNowClick(){sessionStorage.removeItem("SelectedpromoCode");sessionStorage.removeItem("SelectedpromoName");sessionStorage.removeItem("selectedPromoDetailsObj");sessionStorage.removeItem("selectedPromo");window.location=PAGE.hsdSearchPage}
...
function setOfferOftheDayData(a){a=a.responseJSON;var b=function(a){var b=a.imageThumbnail;return'\x3cdiv class\x3d"item promoi" \x3e\x3cdiv class\x3d"over-img"\x3e\n\n                 \x3cpicture\x3e\n                              \x3csource media\x3d"(max-width: 420px)" data-srcset\x3d"'+b+'"\x3e\n                              \x3csource media\x3d"(min-width: 421px)" data-srcset\x3d"'+b+'"\x3e\n                              \x3cimg class\x3d"lazyload" data-src\x3d"'+b+'"  alt\x3d"Offer Image" \x3e\n                           \x3c/picture\x3e\n                    \x3cdiv class\x3d"img-overlay"\x3e\x3c/div\x3e\n              \t\x3c/div\x3e\n              \t\x3cdiv class\x3d"over-btn"\x3e\n                 \t\x3ca class\x3d"btn btn-mob book-btn '+
a.promoClass+'" href\x3d'+a.url+'\x3eAvail the offer\x3c/a\x3e\n\n              \t\x3c/div\x3e\n                \x3cdiv class\x3d"row"\x3e\n                \t\x3cdiv class\x3d"over-content col-xs-12"\x3e\n                    \t\x3cdiv class\x3d"pdlr-15"\x3e \n                        \t\x3ch3 class\x3d"large_plus_text"\x3e\x3ci class\x3d"ng-binding"\x3e'+a.promoName+'\x3c/i\x3e\x3c/h3\x3e\n                            \t\x3ci\x3e\x3cspan class\x3d"ng-binding"\x3e'+a.promoMsg+"\x3c/span\x3e\x3c/i\x3e\n                        \x3c/div\x3e\n                   \x3c/div\x3e\n               \x3c/div\x3e\x3c/div\x3e"};
if(a)if(offerData=[],0<a.length){for(var c=0;c<a.length;c++){var d=PAGE.sterlingOffers+a[c].promoName.toLowerCase().replace(/\s+/g,"-")+"-"+a[c].promoCode.toLowerCase()+".html";a[c].url=encodeURI(d);a[c].promoClass="secondary-link";a[c].promoForLoggedin&&"true"==a[c].promoForLoggedin?(d=session_get())&&d.isLoggedIn&&offerData.push({url:a[c].url,imageThumbnail:a[c].body.imageThumbnail,promoClass:a[c].promoClass,promoName:a[c].promoName,promoMsg:a[c].body.promoMsg}):offerData.push({url:a[c].url,imageThumbnail:a[c].body.imageThumbnail,
promoClass:a[c].promoClass,promoName:a[c].promoName,promoMsg:a[c].body.promoMsg})}a=offerData.map(b).join("");$("#promo-carousel").html(a);$("#promo-carousel").owlCarousel({navigation:!0,items:3,itemsDesktop:[1199,3],itemsDesktopSmall:[979,3]})}else $("#promoOfferOftheDay").hide(),$("#noOfferOfTheDay").show();else $("#promoOfferOftheDay").hide(),$("#noOfferOfTheDay").show()}
...
b.attr("src",c)):(c=b.attr("data-original")+".imgt.1366.600.png",b.attr("src",c))}});$(".lazy-loader").Lazy({effect:"fadeIn",effectTime:1E3,threshold:0,placeholder:"data:image/gif;base64,R0lGODlhEALAPQAPzl5uLr9Nrl8e7..."})});var media_events=[];media_events.loadstart=0;media_events.play=0;media_events.pause=0;
...
-1!=window.location.href.indexOf("/resorts-hotels")&&$(function(){if(!document.getElementById("tab1Data")){var a={};a.resort=$("#resortLink").data("netcoreresortname");a.resortbanner=$("img.destination-banner").attr("src");a.resortbanner||(a.resortbanner=$("img.destination-banner").data("src"));a.resortbanner&&(a.resortbanner=origin+a.resortbanner);a.url=window.location.href;smartech("dispatch","hotel_browse_event",a)}(a=getQueryStringValue("bknowenbl"))&&"rsIH9RfihOdYsYPtpJmF2Q"==a&&($(".ha-header-front .container").remove(),
$(".top-header").remove(),$(".resortHeader").remove(),$(".bookThisRoomBtn").remove(),$(".header").remove(),$(".wrapper").css("padding-top","0px"),$(".link-arrow-next ").remove(),$(".footer-section").remove(),$("#meetings").remove())});
...
$(document).ready(function(){$(document).on("click","#otp-icon-mob-label",function(){$("#memberMobile").val("")});$(document).on("click","#otp-icon-email-label",function(){$("#memberEmail").val("")});$(document).on("click","#loginBtn",function(){$("#loginBtn").disableButton();setSessionData("socialLogin",!1);var a=$("#memberID").val(),b=$("#memberPassword").val();$("#invalidMemberPasswordError").html(UIERRORS.EMPTY);$("#memberLoginError").html(UIERRORS.EMPTY);loginAsVOMember(a,b,callBackFunMemberLogin)});
$("#memberMobile,#memberEmail").keyup(function(a){"13"==(a.keyCode?a.keyCode:a.which)&&($("#signInAsMember").disableButton(),menberLoginOTP("form1"))});$("#memberOTP").keyup(function(a){"13"==(a.keyCode?a.keyCode:a.which)&&($("#signInAsMember").disableButton(),submitMemberOTP())});$(document).on("click","#signInAsMember",function(){$("#signInAsMember").disableButton();$("#memberLoginError").html(UIERRORS.EMPTY);menberLoginOTP("form1")});$(document).on("click","#memberFormResendOTP",function(){menberLoginOTP("form2")});
$(document).on("click","#submitMemberOTP",function(){deleteSession("isQuickPayLogin");submitMemberOTP()});$(document).on("click","#submitMemberOTP2",function(){setSessionData("isQuickPayLogin",!0);submitMemberOTP()});$(document).on("click","#openResetVoPwdModal",function(){$("#modal-authentication").modal("hide");$("#modal-forgot-pwd-email").modal("show");$("#currentUserTypeResetPwd").html("vo")});$("#member_form_username").blur(function(){validateMemberLoginForm("validateAll")});$("#member_form_password").keyup(function(){validateMemberLoginForm("validateAll")});
$("#memberMobile").blur(function(){validateMemberLoginForm("mobile")});$("#memberMobile").keyup(function(){validateMemberLoginForm("mobile")});$("#memberEmail").blur(function(){validateMemberLoginForm("mail")});$("#memberEmail").keyup(function(){validateMemberLoginForm("mail")});$("#memberOTP").blur(function(){validateMemberLoginForm("memberOTP")});$("#memberOTP").keyup(function(){validateMemberLoginForm("memberOTP")});$("#quickPayCheckBox").change(function(){this.checked?$("#quickPaySection").show():
$("#quickPaySection").hide()})});function netcoreMobileFormat(a){var b="";a&&(b=a.replace("+",""),b=b.replace(/\s/g,""));b&&10<b.length&&(a=b.length,b=b.substr(a-10,a));return b}function netcoreMobileCountryCodeFormat(a){var b="";a&&(b=a.replace("+",""),b=b.replace(/\s/g,""));return b&&10<b.length?b.substr(0,b.length-10):""}
...
$("#memberFormPasswordError").html(passwordValidation($("#member_form_password").val())):$("#memberFormPasswordError").html(UIERRORS.EMPTY),0<$("#member_form_password").val().length?$("#signInAsMember").enableButton():$("#signInAsMember").disableButton();"mobile"==a&&customerMobileNumberValidation($("#memberMobile"),$("#memberFormMobileError"),$("#signInAsMember"));"mail"==a&&customerEmailValidation($("#memberEmail"),$("#memberFormMailError"),$("#signInAsMember"));"memberOTP"==a&&customerOTPValidation($("#memberOTP"),
$("#memberFormOTPError"),$("#submitMemberOTP"),$("#submitMemberOTP2"),!0)}
...
function getCustomerPersonalDetails(a,b){http_post(GlobalConstants.DOMAIN_URL+GlobalConstants.URL.HSD.GET_CUSTOMER_DETAILS,function(a){if(a&&a.responseJSON&&a.responseJSON.data){var c=session_get();c.user.profile=a.responseJSON.data;session_set(c);b(a.responseJSON.data)}},{email:a})}
...
$(document).ready(function(){function a(a){var b="";a&&(b=a.replace("+",""),b=b.replace(/\s/g,""));b&&10<b.length&&(a=b.length,b=b.substr(a-10,a));return b}$(document).on("click","#signInAsGuest",function(){if(validateGuestLogin("validateAll")){$("#signInAsGuest").disableButton();var b=$("#guest_form_username").val(),c=$("#guest_form_password").val();loginAsGuest(b,c,function(b){if("OK"==b.statusText||"success"==b.statusText){if($("#signInAsGuest").enableButton(),b.responseJSON&&b.responseJSON.data){try{if(b.responseJSON.data.mobileNo){smartech("identify",
a(b.responseJSON.data.mobileNo));var c=smartech,d=b.responseJSON.data.firstName,g=b.responseJSON.data.lastName,h=b.responseJSON.data.email,k=b.responseJSON.data.mobileNo;guestObj={};guestObj.first_name=d?d:"";guestObj.last_name=g?g:"";guestObj.email=h?h:"";guestObj.mobile=a(k);d=guestObj;g="";k&&(g=k.replace("+",""),g=g.replace(/\s/g,""));var l=g&&10<g.length?g.substr(0,g.length-10):"";d.COUNTRY_CODE=l;c("dispatch","signin",guestObj)}}catch(t){}guestLoginSuccess(b.responseJSON.data)}}else setSessionData("userType",
null),$("#loginInvalidGuestMsg").html(API_ERROR[b.responseJSON.apiError])})}});$(document).on("click","#openResetHsdPwdModal",function(){$("#modal-authentication").modal("hide");$("#modal-forgot-pwd-email").modal("show");$("#currentUserTypeResetPwd").html("hsd")});$("#guest_form_username").blur(function(){validateGuestLogin("validateAll")});$("#guest_form_password").keyup(function(){validateGuestLogin("validateAll")})});
